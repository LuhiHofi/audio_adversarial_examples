"""
Universal adversarial perturbation for Whisper ASR.
This code implements a universal adversarial perturbation attack for Whisper ASR models with Linf PGD attack as the base adversarial attack.
Inspiration was taken from the original UniversalLangAttack from Robust Speech repository. 
It was modified to work as an original universal perturbation attack and to work with arbitrary batch size.
"""
...
MAXLEN=16000*30
class UniversalWhisperPGDAttack(TrainableAttacker,ASRLinfPGDAttack):
    def __init__(self,asr_brain,*args,epochs=10,eps_item=0.001,success_every=10,univ_perturb=None,epoch_counter=None,t=0.8,**kwargs):
        ...
        for epoch in self.epoch_counter:
            ...
            for idx, batch in enumerate(loader):
                ...
                for i in range(self.nb_iter):
                    perturbed_input = wav_init + delta_batch + r
            ...
            if (epoch % self.success_every)==0:
                ...
                if success_rate > best_success_rate:
                    ...
                    print(mean_CER)
                    self.checkpointer.save_and_keep_only()
        print(f"Training finisihed. Best success rate: {best_success_rate*100:.2f}%") 
...
"""
Universal adversarial perturbation for Whisper ASR with added background noise after the training. 
Training is the same as the original UniversalWhisperPGDAttack, but during the perturbation, background noise is added to the perturbed audio.
"""
class UniversalWithNoiseWhisperPGDAttack(UniversalWhisperPGDAttack):
    def __init__(self,asr_brain,*args,epochs=10,eps_item=0.001,success_every=10,univ_perturb=None,epoch_counter=None, background_audio_path="audio_backgrounds/raining-in-backyard.wav", t=0.8, background_noise_strength=0.5, **kwargs):
        super().__init__(asr_brain, *args, epochs=epochs, eps_item=eps_item, success_every=success_every, univ_perturb=univ_perturb, epoch_counter=epoch_counter, t=t, **kwargs)
